...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // pass thhem to the shaders
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glUniformMatrix4fv(viewLoc, 1, GL_FALSE, &view[0][0]);

        // currently we set the projection matrix each frame, but since projection matrix rarely changes it's often set outside the main loop only once.
